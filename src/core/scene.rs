...
pub fn setup(
    ...
    for (img, size, rows, x, y) in [ // list of assets
        ...
        let layout = texture_atlas_layouts.add(TextureAtlasLayout::from_grid(UVec2::splat(size), rows, 1, None, None));
...
pub fn run_animations(time: Res<Time>, mut query: Query<(&mut BaseAnimation, &mut TextureAtlas, &mut Transform)>) {
